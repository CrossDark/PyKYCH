"""
SQLite 数据库模块 — 管理 Markdown 文章的存储与查询。
"""
...
async def list_articles(page: int = 1, per_page: int = 10) -> dict:
    ...
    try:
        ...
        row = await conn.execute_fetchall("SELECT COUNT(*) FROM articles")
...
async def create_article(slug: str, title: str, content: str) -> dict:
    """创建一篇新文章。"""
    conn = await get_db()
    try:
        now = datetime.now(timezone.utc).isoformat()
        await conn.execute(
            "INSERT INTO articles (slug, title, content, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?)",
...
async def update_article(slug: str, title: str, content: str) -> Optional[dict]:
    ...
    try:
        ...
        cursor = await conn.execute(
            "UPDATE articles SET title = ?, content = ?, updated_at = ? "
            "WHERE slug = ?",
...
async def seed_db() -> int:
    ...
    try:
        ...
        for a in SEED_ARTICLES:
            now = datetime.now(timezone.utc).isoformat()
            cursor = await conn.execute(
                "INSERT OR IGNORE INTO articles (slug, title, content, created_at, updated_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (a["slug"], a["title"], a["content"], now, now),
            )
