...
- **状态**: ✅ 已修复（`_safe_redirect_url()` 校验 + 回退到 `/`）
...
- **状态**: ✅ 已修复（`_check()` 自动对 POST 请求验证 CSRF；`admin_base.html` JS 自动注入 CSRF Token 到所有表单）
...
- **状态**: ✅ 已修复（将 `"typst_pages"` 加入删除循环） `delete_user()` 未清理 `featured_articles` 表
...
- **状态**: ✅ 已修复（在级联删除中增加 `featured_articles` 表清理）
...
- **状态**: ✅ 已修复（将 `_write_lock` 扩大到覆盖整个读-改-写流程） `get_setting()` 每次调用都读取文件 — 性能问题
...
- **状态**: ✅ 已修复（添加基于文件 mtime 的内存缓存） 文件上传无磁盘写入错误处理
...
- **修复建议**:
- **状态**: ✅ 已修复（添加 `try/except (OSError, PermissionError)` 包裹） `asyncio.create_task()` 后台任务异常无日志
...
- **状态**: ✅ 已修复（创建 `_create_background_task()` 封装，带 `add_done_callback` 异常日志，替换所有 5 处裸 `create_task`）
...
- **状态**: ✅ 已修复（使用 `conn.begin()` + `commit()` / `rollback()` 包裹所有删除操作） `main.py` 中 `seed_admin` 后重复导入 `logging`
- **文件**: `main.py` 第 86 行
