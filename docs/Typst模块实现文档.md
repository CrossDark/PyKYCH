...
CREATE TABLE IF NOT EXISTS typst_cache (
    ...
    dependencies TEXT DEFAULT NULL,              -- ★ JSON 数组：跨文章引用依赖的 slug 列表
...
**缓存新鲜度判断：** 查询缓存时，通过 `JOIN typst_pages` 并检查 `tc.compiled_at >= tp.updated_at` 确保缓存未被过期。同时检查 `dependencies` 列中记录的所有被引用文章——若任一依赖的 `updated_at > compiled_at`，缓存自动失效。若文章被修改，`updated_at` 会更新，旧缓存自动失效。
...
| `extract_site_imports(source)` | 提取所有跨文章引用 slug（`site:` 语法） |

### 跨文章引用解析

PyKYCH 扩展了 Typst 的导入机制，支持通过 `site:` 协议引用网站上的其他 Typst 文章。此功能在编译前自动将虚拟引用解析为实际文件。

**语法：**

```typst
#import "site:slug"        // 导入其他文章（获取其导出的变量/函数）
#include "site:slug"       // 直接包含其他文章的全部内容
```
...
**递归解析流程 `_resolve_site_imports()`：**

```mermaid
flowchart TD
    A[接收 Typst 源码] --> B{_SITE_IMPORT_RE 匹配?}
    B -->|❌ 无匹配| Z[返回原始源码]
    B -->|✅ 匹配到 site:slug| C{slug 在 visited 中?}
    C -->|✅ 已访问| D[跳过 — 防止循环引用]
    C -->|❌ 未访问| E[标记 visited.add slug]
    E --> F[DB 查询 typst_pages WHERE slug=...]
    F --> G{文章存在?}
    G -->|❌ 不存在| H[保留原始引用 — 由 typst 报错]
    G -->|✅ 存在| I[获取文章内容 + 辅助文件]
    I --> J[生成安全目录名: _site_&lt;slug&gt;]
    J --> K[替换源码中的 site:slug → 相对路径]
    K --> L[递归解析被引用文章的 site: 引用]
    L --> M[收集: index.typ + config.typ + 辅助文件]
    M --> N{还有更多匹配?}
    N -->|✅ 是| B
    N -->|❌ 否| O[返回 修改后源码, 额外辅助文件列表]
```
...
├── index.typ                  ← 主文章（site: 引用已被替换为相对路径）
├── config.typ                  ← 共享配置
├── lib/                        ← 主文章的辅助文件
│   └── utils.typ
├── _site_math-lib/             ← ★ 被引用文章 math-lib 的独立目录
│   ├── index.typ               ← 被引用文章内容（嵌套引用已递归解析）
│   ├── config.typ              ← 共享配置副本
│   └── helpers.typ             ← 被引用文章的辅助文件
├── _site_shared-footer/        ← ★ 被引用文章 shared-footer 的独立目录
│   ├── index.typ
│   └── config.typ
├── .cache/
└── .runtime/
...
| 被引用文章位置 | site: 引用出现的文件位置 | 替换后的相对路径 |
|---|---|---|
| `_site_math-lib/` | 主文章 `index.typ`（顶层） | `_site_math-lib/index.typ` |
| `_site_shared-utils/` | 已导入文章 `_site_math-lib/index.typ` | `../_site_shared-utils/index.typ` |
| `_site_deep-lib/` | 二层嵌套 | `../../_site_deep-lib/index.typ` |

路径计算规则：`base_dir` 为空时直接用目录名，否则用 `../` 前缀拼接。Typst 从当前文件位置按相对路径解析，因此嵌套导入天然正确。

**关键函数：**

| 函数 | 用途 |
|------|------|
| `extract_site_imports(source)` | 从源码中提取所有 `site:` 引用的 slug 列表 |
| `_get_article_content(slug)` | 从 `typst_pages` 表查询文章内容 |
| `_resolve_site_imports(source, base_dir, visited)` | **递归核心**：解析 site 引用、替换路径、收集辅助文件。返回 `(修改后源码, 额外辅助文件列表)` |

**编译流程中的集成点：**

`compile_typst_to_html()` 和 `compile_typst_to_pdf()` 在构建工作区之前调用 `_resolve_site_imports()`：
...
# 后续工作区构建使用 resolved_source 和 all_aux
_write_workspace(workspace, resolved_source, all_aux, shared_config)
```

**循环引用防护：**

`visited` 集合在递归过程中跨层级共享。一旦某个 slug 被处理过，后续再遇到同一 slug 时直接跳过，避免无限递归。例如 A → B → A 时，第二次遇到 A 会被 `visited` 拦截。

**缓存依赖追踪：**

编译缓存不仅存储 HTML/PDF，还记录依赖关系：

1. **缓存写入** — `build_and_cache_typst()` 调用 `extract_site_imports(source)` 提取依赖 slug，以 JSON 数组存入 `typst_cache.dependencies`
2. **缓存读取** — `get_cached_typst_html()` / `get_cached_typst_pdf()` 在比对 `compiled_at >= updated_at` 后，额外遍历所有依赖 slug，逐一比对 `updated_at`；任一依赖更新则缓存失效
3. **自动迁移** — `_migrate_typst_cache_dependencies()` 在 `init_tables()` 中为旧缓存表添加 `dependencies` 列

这确保当被引用文章更新后，引用者的缓存自动失效，下次访问时重新编译。
